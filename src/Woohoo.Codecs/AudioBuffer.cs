...
namespace CUETools.Codecs
{
    ...
    public class AudioBuffer
    {
        ...
        {
            length = _length;
            size = _bytes.Length / this.PCM.BlockAlign;
            bytes = _bytes;
            dataInSamples = false;
            dataInBytes = true;
            dataInFloat = false;
        ...
        {
            if (dataInBytes)
                Buffer.BlockCopy(src.Bytes, srcOffset * pcm.BlockAlign, this.Bytes, dstOffset * pcm.BlockAlign, copyLength * pcm.BlockAlign);
            if (dataInSamples)
                Buffer.BlockCopy(src.Samples, srcOffset * pcm.ChannelCount * 4, this.Samples, dstOffset * pcm.ChannelCount * 4, copyLength * pcm.ChannelCount * 4);
            if (dataInFloat)
                Buffer.BlockCopy(src.Float, srcOffset * pcm.ChannelCount * 4, this.Float, dstOffset * pcm.ChannelCount * 4, copyLength * pcm.ChannelCount * 4);
        }
        ...
        public unsafe void Prepare(AudioBuffer _src, int _offset, int _length)
        {
            ...
            if (_src.dataInBytes)
                ...
                dataInFloat = true;
            this.Load(0, _src, _offset, length);
        }
        ...
        unsafe public void Interlace(int pos, int* src1, int* src2, int n)
        {
            ...
            if (this.PCM.BitsPerSample == 16)
            {
                fixed (byte* bs = this.Bytes)
                {
                    int* res = ((int*)bs) + pos;
                    for (int i = n; i > 0; i--)
            ...
            else if (this.PCM.BitsPerSample == 24)
            {
                fixed (byte* bs = this.Bytes)
                {
                    byte* res = bs + pos * 6;
