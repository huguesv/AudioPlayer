...
internal sealed class LzmaEncoder
{
    ...
    class LenEncoder
    {
        RangeCoder.RangeBitEncoder _choice = new RangeCoder.RangeBitEncoder();
        RangeCoder.RangeBitEncoder _choice2 = new RangeCoder.RangeBitEncoder();
        RangeCoder.RangeBitTreeEncoder[] _lowCoder = new RangeCoder.RangeBitTreeEncoder[LzmaBase.kNumPosStatesEncodingMax];
        RangeCoder.RangeBitTreeEncoder[] _midCoder = new RangeCoder.RangeBitTreeEncoder[LzmaBase.kNumPosStatesEncodingMax];
        RangeCoder.RangeBitTreeEncoder _highCoder = new RangeCoder.RangeBitTreeEncoder(LzmaBase.kNumHighLenBits);

        public LenEncoder()
        {
            for (uint posState = 0; posState < LzmaBase.kNumPosStatesEncodingMax; posState++)
            {
                _lowCoder[posState] = new RangeCoder.RangeBitTreeEncoder(LzmaBase.kNumLowLenBits);
                _midCoder[posState] = new RangeCoder.RangeBitTreeEncoder(LzmaBase.kNumMidLenBits);
            }
        }
    ...
    class LenPriceTableEncoder : LenEncoder
    {
        ...
        {
            this.SetPrices(posState, _tableSize, _prices, posState * LzmaBase.kNumLenSymbols);
    ...
    LZ.LzBinTree _matchFinder = null;
    RangeCoder.RangeEncoder _rangeEncoder = new RangeCoder.RangeEncoder();

    RangeCoder.RangeBitEncoder[] _isMatch = new RangeCoder.RangeBitEncoder[LzmaBase.kNumStates << LzmaBase.kNumPosStatesBitsMax];
    RangeCoder.RangeBitEncoder[] _isRep = new RangeCoder.RangeBitEncoder[LzmaBase.kNumStates];
    RangeCoder.RangeBitEncoder[] _isRepG0 = new RangeCoder.RangeBitEncoder[LzmaBase.kNumStates];
    RangeCoder.RangeBitEncoder[] _isRepG1 = new RangeCoder.RangeBitEncoder[LzmaBase.kNumStates];
    RangeCoder.RangeBitEncoder[] _isRepG2 = new RangeCoder.RangeBitEncoder[LzmaBase.kNumStates];
    RangeCoder.RangeBitEncoder[] _isRep0Long = new RangeCoder.RangeBitEncoder[LzmaBase.kNumStates << LzmaBase.kNumPosStatesBitsMax];

    RangeCoder.RangeBitTreeEncoder[] _posSlotEncoder = new RangeCoder.RangeBitTreeEncoder[LzmaBase.kNumLenToPosStates];

    RangeCoder.RangeBitEncoder[] _posEncoders = new RangeCoder.RangeBitEncoder[LzmaBase.kNumFullDistances - LzmaBase.kEndPosModelIndex];
    RangeCoder.RangeBitTreeEncoder _posAlignEncoder = new RangeCoder.RangeBitTreeEncoder(LzmaBase.kNumAlignBits);
    ...
    void Create()
    {
        ...
        {
            LZ.LzBinTree bt = new LZ.LzBinTree();
            int numHashBytes = 4;
        ...
        if (_dictionarySize == _dictionarySizePrev && _numFastBytesPrev == _numFastBytes)
            return;
        _matchFinder.Create(_dictionarySize, kNumOpts, _numFastBytes, LzmaBase.kMatchMaxLen + 1 + kNumOpts);
    ...
    uint GetOptimum(uint position, out uint backRes)
    {
        ...
        {
            uint shortRepPrice = repMatchPrice + this.GetRepLen1Price(_state, posState);
        ...
        for (i = 0; i < LzmaBase.kNumRepDistances; i++)
        {
            ...
            if (repLen < 2)
                continue;
            uint price = repMatchPrice + this.GetPureRepPrice(i, _state, posState);
            do
            {
    ...
    public void CodeOneBlock(out long inSize, out long outSize, out bool finished)
    {
        ...
        while (true)
        {
            ...
            uint pos;
            uint len = this.GetOptimum((uint)nowPos64, out pos);
