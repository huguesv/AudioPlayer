...
internal class HuffmanDecoder
{
    ...
    */
    public HuffmanDecoder(uint numcodes, byte maxbits, BitStream bitbuf, ushort[] buffLookup = null)
    {
        ...
        this.lookup = buffLookup == null ? (new ushort[(1 << maxbits)]) : buffLookup;

        this.huffnode = new node_t[numcodes];
    ...
    */
    public uint DecodeOne()
    {
        ...
        uint bits = this.bitbuf.Peek(this.maxbits);

        /* look it up, then remove the actual number of bits for this code */
        uint lookup = this.lookup[bits];
        this.bitbuf.Remove((int)(lookup & 0x1f));
    ...
    */
    public huffman_error ImportTreeRLE()
    {
        ...
        for (curnode = 0; curnode < this.numcodes;)
        {
            /* a non-one value is just raw */
            int nodebits = (int)this.bitbuf.Read(numbits);
            if (nodebits != 1)
            {
                this.huffnode[curnode++].numbits = (byte)nodebits;
            }
            ...
            else
            {
                ...
                nodebits = (int)this.bitbuf.Read(numbits);
                if (nodebits == 1)
                {
                    this.huffnode[curnode++].numbits = (byte)nodebits;
                }
                ...
                {
                    int repcount = (int)this.bitbuf.Read(numbits) + 3;
                    if (repcount + curnode > this.numcodes)
                    {
        ...
        error = this.AssignCanonicalCodes();
        ...
        return this.bitbuf.Overflow() ? huffman_error.HUFFERR_INPUT_BUFFER_TOO_SMALL : huffman_error.HUFFERR_NONE;
    }
    ...
    */
    public huffman_error ImportTreeHuffman()
    {
        ...
        HuffmanDecoder smallhuff = new HuffmanDecoder(24, 6, this.bitbuf);
        smallhuff.huffnode[0].numbits = (byte)this.bitbuf.Read(3);
        start = (int)this.bitbuf.Read(3) + 1;
        ...
        for (curcode = 0; curcode < this.numcodes;)
        {
            int value = (int)smallhuff.DecodeOne();
            if (value != 0)
            {
                this.huffnode[curcode++].numbits = (byte)(last = value - 1);
            }
            else
            {
                count = (int)this.bitbuf.Read(3) + 2;
                if (count == 7 + 2)
                {
                    count += (int)this.bitbuf.Read(rlefullbits);
                }

                for (; count != 0 && curcode < this.numcodes; count--)
                {
                    this.huffnode[curcode++].numbits = (byte)last;
                }
            }
        }
    ...
    */
    private huffman_error AssignCanonicalCodes()
    {
        ...
        for (curcode = 0; curcode < this.numcodes; curcode++)
        {
            ...
            node_t node = this.huffnode[curcode];
    ...
    private void BuildLookupTable()
    {
        ...
        for (curcode = 0; curcode < this.numcodes; curcode++)
        {
            ...
            if (node.numbits > 0)
            {
                ...
                shift = this.maxbits - node.numbits;
                dest = node.bits << shift;
                destend = ((node.bits + 1) << shift) - 1;
                while (dest <= destend)
                {
                    this.lookup[dest++] = (ushort)value;
                }
            }
        }
    }
