...
internal static partial class ChdHunkReaders
{
    ...
    */
    ...
    internal static void avHuff(byte[] buffIn, int buffInLength, byte[] buffOut, int buffOutLength, ChdCodec codec)
    {
        ...
        {
            // decode the audio
            DecodeAudio(audioChannels, audioSamplesPerBlock, buffIn, buffInIndex, audioHuffmanTreeSize, audioChannelCompressedSize, buffOut, audioChannelDestStart, codec);
        ...
        if (videoWidth > 0 && videoHeight > 0)
        {
            ...
            // decode the video
            DecodeVideo(videoWidth, videoHeight, buffIn, buffInIndex, (uint)buffInLength - buffInIndex, buffOut, videoDestStart, videostride, codec);
        }
    ...
    private static void DecodeAudio(uint channels, uint samples, byte[] buffIn, uint buffInOffset, uint treesize, uint?[] audioChannelCompressedSize, byte[] buffOut, uint?[] audioChannelDestStart, ChdCodec codec)
    {
    ...
    private static void DecodeVideo(uint width, uint height, byte[] buffIn, uint buffInOffset, uint buffInLength, byte[] buffOut, uint buffOutOffset, uint dstride, ChdCodec codec)
    {
