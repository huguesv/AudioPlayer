...
namespace CUETools.Codecs.Flake
{
    ...
    public class AudioDecoder : IAudioSource
    {
        ...
        public AudioDecoder(DecoderSettings settings, string path, Stream IO = null)
        {
            ...
            this.decode_metadata();

            frame = new FlacFrame(this.PCM.ChannelCount);
            framereader = new BitReader();

            //max_frame_size = 16 + ((Flake.MAX_BLOCKSIZE * PCM.BitsPerSample * PCM.ChannelCount + 1) + 7) >> 3);
            if (((int)max_frame_size * this.PCM.BitsPerSample * this.PCM.ChannelCount * 2 >> 3) > _framesBuffer.Length)
            {
                byte[] temp = _framesBuffer;
                _framesBuffer = new byte[((int)max_frame_size * this.PCM.BitsPerSample * this.PCM.ChannelCount * 2 >> 3)];
            ...
            if (this.PCM.BitsPerSample != 16 && this.PCM.BitsPerSample != 24)
                throw new Exception("invalid flac file");

            samplesBuffer = new int[FlakeConstants.MAX_BLOCKSIZE * this.PCM.ChannelCount];
            residualBuffer = new int[FlakeConstants.MAX_BLOCKSIZE * this.PCM.ChannelCount];
        }
        ...
        public TimeSpan Duration => this.Length < 0 ? TimeSpan.Zero : TimeSpan.FromSeconds((double)this.Length / this.PCM.SampleRate);
        ...
        public long Position
        {
            ...
            set
            {
                ...
                if (value < this.Position || value > _sampleOffset)
                {
                ...
                while (value > _sampleOffset)
                {
                    ...
                    this.fill_frames_buffer();
                    if (_framesBufferLength == 0)
                        throw new Exception("seek failed");

                    int bytesDecoded = this.DecodeFrame(_framesBuffer, _framesBufferOffset, _framesBufferLength);
        ...
        public int Read(AudioBuffer buff, int maxLength)
        {
            ...
            while (_samplesInBuffer < sampleCount)
            {
                ...
                {
                    this.interlace(buff, offset, _samplesInBuffer);
            ...
            this.interlace(buff, offset, sampleCount);
        ...
        unsafe void decode_frame_header(BitReader bitreader, FlacFrame frame)
        {
            ...
            if (FlakeConstants.flac_bitdepths[bps_code] != this.PCM.BitsPerSample)
            ...
            if (frame_channels != this.PCM.ChannelCount)
                throw new Exception("invalid channel mode");
        ...
        unsafe void decode_subframe_fixed(BitReader bitreader, FlacFrame frame, int ch)
        {
            ...
            // residual
            this.decode_residual(bitreader, frame, ch);
        }
        ...
        unsafe void decode_subframes(BitReader bitreader, FlacFrame frame)
        {
            fixed (int* r = residualBuffer, s = samplesBuffer)
                for (int ch = 0; ch < this.PCM.ChannelCount; ch++)
                {
                    ...
                    frame.subframes[ch].obits = this.PCM.BitsPerSample - frame.subframes[ch].wbits;
                    ...
                    switch (frame.subframes[ch].best.type)
                    {
                        case SubframeType.Constant:
                            this.decode_subframe_constant(bitreader, frame, ch);
                            break;
                        case SubframeType.Verbatim:
                            this.decode_subframe_verbatim(bitreader, frame, ch);
                            break;
                        case SubframeType.Fixed:
                            this.decode_subframe_fixed(bitreader, frame, ch);
                            break;
                        case SubframeType.LPC:
                            this.decode_subframe_lpc(bitreader, frame, ch);
                            break;
                        default:
            ...
            for (int ch = 0; ch < this.PCM.ChannelCount; ch++)
            {
                switch (frame.subframes[ch].best.type)
                {
                    ...
                    case SubframeType.Verbatim:
                        ...
                        break;
                    case SubframeType.Fixed:
                        this.restore_samples_fixed(frame, ch);
                        break;
                    case SubframeType.LPC:
                        this.restore_samples_lpc(frame, ch);
                        break;
                }
                if (frame.subframes[ch].wbits != 0)
                {
        ...
        public unsafe int DecodeFrame(byte[] buffer, int pos, int len)
        {
            fixed (byte* buf = buffer)
            {
                ...
                this.decode_frame_header(framereader, frame);
                this.decode_subframes(framereader, frame);
                framereader.flush();
